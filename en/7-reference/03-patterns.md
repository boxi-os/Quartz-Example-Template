...
1. **Sidebar headings** are small, capitalised and muted — they are labelling, not content. The
   same form stands in nine stylesheets, and that is intended: every page in chapter 5 is to show
...
3. **Borders of controls** draw their colour from `--tpl-rule-control`, not from `--lightgray`.
   The faint tone is enough for a divider, but not for the border of something you operate — WCAG
   demands 3:1 there.
...
7. **All fold-out chevrons are the same chevron** from the Lucide set — in the explorer, in the
   table of contents, in the collapsible layout box.
...
The nine panel headings and the seven hover rules could be pulled into one shared file. That would
