...
A Quartz site consists of **components**: the explorer, the search, the table of contents, the
breadcrumbs, the footer — each built by a plugin, each with a look of its own. This chapter goes
...
A **stylesheet** is the file in which the look is described — in the language CSS, here in its
extension SCSS. This template has 30 of them, one per component, and every page here names its
...
| Section | Components |
| --- | --- |
| [[en/5-design/01-navigation/index\|5.1 Navigation]] | explorer, search, colour scheme, reader mode, header, footer, language switcher |
| [[en/5-design/02-above-the-content/index\|5.2 Above the content]] | breadcrumbs, title and date, properties, tags |
| [[en/5-design/03-in-the-content/index\|5.3 In the content]] | body text, callouts, code, tables and media |
| [[en/5-design/04-beside-the-content/index\|5.4 Beside the content]] | table of contents, backlinks, graph, recently changed |
...
typefaces, variables, page grids, accessibility — is chapter [[en/6-adapting/index|6 Adapting]].

## The three decisions everything else rests on

1. **The width decides the grid** — the frame, in one place, not the paragraph on top of it.
2. **Colours are measured.** 89 pairs, all above the WCAG threshold, in both modes.
