...
von Quartz oder eines Plugins, die eine Vorlage nicht beheben, nur umgehen kann; ein paar sind
...
| Ein Tag im Fließtext hat einen kaputten Link — ein Fehler in Quartz | Tags ins Frontmatter | [[2-formatierung/13-besonderes/pfeile-und-emoji\|Pfeile und Emoji]] |
| Ein Bild hat entweder eine Größe oder einen Alternativtext, nicht beides | Größe über HTML setzen, wenn beides nötig ist | [[2-formatierung/12-medien/bilder\|Bilder]] |
...
| Mermaid ignoriert fast alle Farben der Vorlage; jede eigene Regel braucht `!important` | das mitgelieferte Stylesheet, das genau das tut | [[6-anpassen/01-etwas-aendern\|6.1 Etwas ändern]] |

## Bases, Canvas, Zeichnungen

| Grenze | Was stattdessen | Wo |
| --- | --- | --- |
| Bases sprechen Englisch: „Showing 45 of 45 entries“ und englische Spaltenköpfe kommen aus dem Plugin und lassen sich nicht übersetzen | die Zeile ist leise gesetzt statt entfernt | [[3-obsidian-formate/01-bases/index\|3.1 Bases]] |
...
| Grenze | Was stattdessen | Wo |
| --- | --- | --- |
| Der Explorer startet zugeklappt; die Option `folderDefaultState: open` wird vom Plugin nicht gelesen | offen sind die Ordner auf dem Weg zur aktuellen Seite | [[5-gestaltung/01-navigation/explorer\|Explorer]] |
...
| Die Oberfläche — Explorer, Suche, Rückverweise, Graph — spricht site-weit eine Sprache; Suche, Graph und „Zuletzt geändert“ mischen beide Sprachen; es gibt eine Fehlerseite für beide | die Layout-Boxen sprechen beide Sprachen; ein Build je Sprache würde den Rest lösen | [[6-anpassen/07-zwei-sprachen/grenzen\|Wo die zwei Sprachen aufhören]] |
...
| Das Datum einer Seite kommt nicht aus git, weil `content/` ein Symlink ist — ein Fehler im Plugin | Datum im Frontmatter | [[4-seiten-steuern/01-titel-beschreibung-datum\|4.1 Titel, Beschreibung, Datum]] |
...
| Der Baustein Übersetzungen erreicht die Überschriften der Komponenten nicht | — | [[6-anpassen/08-das-vorlagenpaket\|6.8 Das Vorlagenpaket]] |
| Snapshots enthalten den Inhalt nicht, wenn `content/` ein Symlink ist | der Vault sichert sich über sein eigenes git | [[1-einstieg/03-von-der-notiz-zur-website\|1.3 Von der Notiz zur Website]] |
